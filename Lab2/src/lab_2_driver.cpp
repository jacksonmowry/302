...
uint32_t a502b9de() {
  ...
  sum = (uint64_t)2111111111UL * (uint64_t)X[3] +
        (uint64_t)1492 * (uint64_t)(X[2]) + (uint64_t)1776 * (uint64_t)(X[1]) +
        (uint64_t)5115 * (uint64_t)(X[0]) + (uint64_t)X[4];
  X[3] = X[2];
  X[2] = X[1];
  X[1] = X[0];
  X[4] = (uint32_t)(sum >> 32);
...
bool am_i_greater_ll(unsigned long long v) { return (v > n); }
...
bool am_i_greater_d(double v) { return (v > d); }
  ...
  if (cin >> s >> sel) {
    ...
    if (sel == 0) {
      ...
      if (s % 20 == 10) {
        ...
        d = (double)a502b9de();
        d /= (double)a502b9de();
        while (d > 1000000000.0)
          d /= 10.0;
      }
      rvd = whats_my_number_d();
      printf("%.10lf %s\n", d,
             (rvd - d < 0.000000001 && rvd - d > -0.000000001) ? "Correct"
                                                               : "Incorrect");
      if (printans) {
