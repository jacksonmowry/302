...
void City_Map::Spit_Jgraph() {
  ...
  printf("newgraph xaxis size 8 nodraw min %lf max %lf yaxis nodraw size 8 min "
         "%lf max %lf\n",
         minx, maxx, miny, maxy);
  ...
  for (adjit = path.begin(); adjit != path.end(); adjit++) {
    rs = *adjit;
    printf("newline linethickness 2 marktype circle color 1 0 0 rarrow acfill "
           "1 0 0 pts %lf %lf %lf %lf\n",
           rs->from->x, rs->from->y, rs->to->x, rs->to->y);
  }
}
...
void City_Map::Print() {
  ...
  for (ait = all.begin(); ait != all.end(); ait++) {
    ...
    printf("%7d : Intersection: %4d %4d - %12.6lf %12.6lf - %12.6lf %12.6lf\n",
           i, l->street, l->avenue, l->x, l->y, l->green[STREET],
           l->green[AVENUE]);
    i++;
    for (adjit = l->adj.begin(); adjit != l->adj.end(); adjit++) {
      rs = *adjit;
      ...
      printf("%7d :    Segment to %4d %4d       Distance %12.6lf\n", i,
             rs->to->street, rs->to->avenue, rs->distance);
    }
    i++;
  }
  ...
  for (adjit = path.begin(); adjit != path.end(); adjit++) {
    ...
    printf("%7d : PATH: [%04d,%04d] -> [%04d,%04d] - Time: %12.6lf\n", i,
           rs->from->street, rs->from->avenue, rs->to->street, rs->to->avenue,
           rs->to->best_time);
    i++;
  }
}
...
void usage() {
  cerr << "usage: city_map none|best|worst|avg time|print|jgraph - map on "
          "standard input\n";
  exit(1);
}
